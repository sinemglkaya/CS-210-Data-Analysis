...
def download_links_from_index():
    ...
    firstURL = 'https://www.gov.bm/news'
    
    resp = requests.get(firstURL)
    soup = bs(resp.text, 'lxml')
    
    for item in soup.find_all("ul", {"class": "news-list"}):
        #print(item)
        for li in item.find_all('li'):
            link = li.find("span", {"class": "field-content"}).find('a')['href']
            #print(link)
            
            # Save the collected url in the variable "collected_url"
            collected_url = 'https://www.gov.bm/' + link
    ...
    rootURL = 'https://www.gov.bm/news?page='
    
    for pid in range(start_page,562):
        ...
        for item in soup.find_all("ul", {"class": "news-list"}):
            ...
            for li in item.find_all('li'):
                ...
                if collected_url not in downloaded_url_list:
                    print("\t", collected_url, flush=True)
                    save_link(collected_url, page + 1)
    #########################################
